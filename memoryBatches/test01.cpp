...
int runKernel() {
	...
	status = clEnqueueNDRangeKernel(commandQueue,  kernel, 1, NULL, globalWorkSize, NULL, 2, writeEventsArray2, &kernelEvent1);
...
int main(int argc, char **argv) {
	...
	for (int i = 0; i < 2; i++) {
	  ...
	  cl_event events2[] = { writeEvent3, writeEvent4 };
		...
		writeBuffer1(size * sizeof(float), size, 0, NULL, &writeEvent2);
		runKernel();
